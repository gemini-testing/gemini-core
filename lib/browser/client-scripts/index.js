...
function prepareScreenshotUnsafe(selectors, opts) {
    ...
    var rect = getCaptureRect(selectors, allowViewportOverflow);
    if (rect.error) {
        return rect;
    }
...
function getCaptureRect(selectors, allowViewportOverflow) {
    ...
    for (var i = 0; i < selectors.length; i++) {
        ...
        elementRect = getElementCaptureRect(element, allowViewportOverflow);
        if (elementRect) {
...
function getElementCaptureRect(element, allowViewportOverflow) {
    ...
    var elementRect = getExtRect(css, clientRect, allowViewportOverflow);

    util.each(pseudo, function(pseudoEl) {
        css = lib.getComputedStyle(element, pseudoEl);
        elementRect = elementRect.merge(getExtRect(css, clientRect, allowViewportOverflow));
    });
...
function scrollToCaptureAreaInSafari(viewportCurr, captureArea) {
    ...
    window.scrollTo(viewportCurr.left, Math.min(captureArea.top, maxScrollByY));
