...
module.exports = class Camera {
    static create(screenshotMode, takeScreenshot, browser) {
        return new this(screenshotMode, takeScreenshot, browser);
    }

    constructor(screenshotMode, takeScreenshot, browser) {
    ...
    _cropToViewport(image, page) {
        ...
        const isAndroidMobile = this._browser && this._browser.publicAPI.isMobile && this._browser.publicAPI.isAndroid;
        const cropArea = _.clone(page.viewport);

        if (!isFullPage || isAndroidMobile) {
            _.extend(cropArea, {
                top: 0,
                left: 0
            });
