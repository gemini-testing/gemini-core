...
module.exports = class SetsBuilder {
    ...
    build(projectRoot, globOpts = {}, fileExtensions = FILE_EXTENSIONS) {
        ...
        return this._transformDirsToMasks()
            ...
            .then((expandedFiles) => this._validatePassedMasks(expandedFiles))
    ...
    _validatePassedMasks(expandedFiles) {
        if (!_.isEmpty(this._filesToUse) && _.isEmpty(expandedFiles)) {
            throw new Error(`Did not find files with the specified masks: ${this._filesToUse}`);
        }
