...
export type TestSetsByPlatform = {
    [platform in keyof SetsConfig]: TestSet
}
...
export default class SetsBuilder {
    ...
    public useSets(setsToUse: Array<string>): this {
    ...
    public useFiles(files: Array<string>): this {
        if (!_.isEmpty(files)) {
    ...
    public useBrowsers(browsers: Array<string>): this {
        _.forEach(this._sets, (set) => set.useBrowsers(browsers));

        return this;
    }

    public async build(projectRoot: string, globOpts: GlobOpts = {}, fileExtensions: Array<string> = FILE_EXTENSIONS): Promise<SetCollection> {
